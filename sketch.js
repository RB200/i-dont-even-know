...
function keyPressed(){
    ...
    if(keyCode === 13){
        ...
        if(nextLevel === 1 && q[0] === 4 && q[1] === 2){
            ...
            goToLevel(4,lvlorientation,side)
        }
        else if(nextLevel === 5 && q[0] === 0 && q[1] === 4){
            goToLevel(5,lvlorientation)
        }  
    }
...
function goToLevel(lvl,orientation,side){
    clear()
    level = lvl
    nextLevel = lvl + 1
    background(51)
    if(data.levelData[`level${lvl}`].shapes[`pc${lvl + 1}`].x === 0){
        data.levelData[`level${lvl}`].shapes[`pc${lvl + 1}`] = {
            x: 1700,
            y: 500,
            width: 200,
            height:100
    ...
    var boxDetails = getSquareDetails(lvl,`pc${lvl + 1}`)
    var goalDetails = getSquareDetails(lvl,`level${lvl}goal`)
    strokeWeight(3)
    textSize(40)
    fill('#03dbfc')
    if(lvl === 3){
        ...
        fill(255,255,255)
        text(data.levelData[`level${lvl}`].infotext,250,100)
        fill('#03dbfc')
    }
    
    text(data.levelData[`level${lvl}`].text,1250,100)
    strokeWeight(5)
    ...
    fill(255,0,0)
    var pc1 = rect(boxDetails[0],boxDetails[1],boxDetails[2],boxDetails[3])
    isMouseDownOver(`pc${lvl + 1}`, mouseX, mouseY)
    if(mouseDownShapeTargeted){
        data.levelData[`level${lvl}`].shapes[`pc${lvl + 1}`].x = mouseX
        data.levelData[`level${lvl}`].shapes[`pc${lvl + 1}`].y = mouseY
    }
}